...
# Save the plot as a high quality PNG
ggsave("Publications per year_2024.png", dpi = 600)
...
###############################################################################
#### plot the citations for NOAA 'Omics publications ####
## 2023-09-11 ##
## GFolder where notes and details on gathering these data are here:
## https://drive.google.com/drive/folders/11YUdc7JdGA3k4iOCSECb7CNjHd8I_ha3?usp=drive_link

## previously had plotted these in September but the below is updated from a query run in
## October

setwd("/Volumes/krista.nichols/Documents/NOAA admin/Omics_Working_Group/citations")

#### 2023-10-12 PLOT data downloaded from GDrive to look simply at pubs per year ####
library(tidyverse)
library(readxl)

# this file includes both WoS results and the added eDNA articles added manually
NOAA_omicsPub_1023 <- read_xlsx(path = "All_Omics_Publications_18OCT2023_curated.xlsx")

# initial plot by year of publication
# notice that there are a few from 2019 that slipped through the cracks -- should only be 2020 onward
ggplot(NOAA_omicsPub_1023 %>% filter(`Publication Year` > 2019), aes(`Publication Year`)) + geom_bar() +
  ggtitle("NOAA 'Omics publications by year")

ggsave("All_Omics_Publications_18OCT2023.png", dpi = 600)

ggplot(NOAA_omicsPub_1023 %>% filter(`Publication Year` > 2019), aes(`Publication Year`, y = cumsum(after_stat(count)))) +
         geom_bar()

ggplot(NOAA_omicsPub_1023 %>% filter(`Publication Year` > 2019), 
       aes(x = as.numeric(`Publication Year`), y = cumsum(after_stat(count)))) +
  geom_point()

         
# look in 'Affiliations' for NOAA or 'National Oceanic' and create a new variable 'NOAA affiliation'
# in Web of Science, we searched this text to find the original data:
# National Oceanic Atmospheric Admin (NOAA) - USA), NOAA, National Oceanic Atmospheric Administration
# if not NOAA Affiliated, then we can assume it's NOAA funded
NOAA_omicsPub_1023 <- NOAA_omicsPub_1023 %>%
  mutate(NOAA_affiliated = as.integer(str_detect(Affiliations, pattern = "NOAA|National Oceanic")))

NOAA_omicsPub_1023 %>% filter(NOAA_affiliated == 1) %>% count() # 446 are NOAA affiliated
NOAA_omicsPub_1023 %>% filter(NOAA_affiliated == 0) %>% count() # 220 are NOAA funded 

ggplot(NOAA_omicsPub_1023 %>% filter(`Publication Year` > 2019), aes(`Publication Year`, fill = as.factor(NOAA_affiliated))) + 
  geom_bar(position = "stack") + scale_fill_manual(labels = c("NOAA funded", "NOAA affiliated"), values = c("gray", "black")) +
  theme_bw() + theme(legend.title = element_blank()) + 
  ggtitle("NOAA 'Omics publications by year")

ggsave("NOAApubs_byaffilationvsfunding_101923.png", dpi = 600)

#### 2024-05-30 updated search -- first just the WoS data with refined keywords ####
setwd("/Volumes/krista.nichols/Documents/NOAA admin/Omics_Working_Group/citations")
...
# this file includes both WoS results and the added eDNA articles added manually
NOAA_omicsPub_WoSonly_0524 <- read_xls(path = "20240529_refinedv1_WoS_search.xls")

# initial plot by year of publication
# notice that there are a few from 2019 that slipped through the cracks -- should only be 2020 onward
ggplot(NOAA_omicsPub_WoSonly_0524 %>% filter(`Publication Year` > 2019), aes(`Publication Year`)) + geom_bar() +
  ggtitle("NOAA 'Omics publications by year")

ggsave("All_Omics_Publications_052924.png", dpi = 600)

ggplot(NOAA_omicsPub_WoSonly_0524 %>% filter(`Publication Year` > 2019), aes(`Publication Year`, y = cumsum(after_stat(count)))) +
  geom_bar()


# from those variables, find NOAA funded but not NOAA affiliated pubs (those that were provided to external partners)
NOAA_omicsPub_WoSonly_0524 <- NOAA_omicsPub_WoSonly_0524 %>%
  mutate(NOAA_affiliated = as.integer(str_detect(Affiliations, pattern = "NOAA|National Oceanic")))

# there are 554 publications originally but only 549 that are either NOAA affiliated or NOAA funded
# what are the other 5 that are missing in these filters? checked this and then manually added affiliations for each
NOAA_omicsPub_WoSonly_0524 %>% filter(NOAA_affiliated == 1) %>% count() # 348 are NOAA affiliated
NOAA_omicsPub_WoSonly_0524 %>% filter(NOAA_affiliated == 0) %>% count() # 206 are NOAA funded 
NOAA_affiliated <- NOAA_omicsPub_WoSonly_0524 %>% filter(NOAA_affiliated == 1)
NOAA_funded <- NOAA_omicsPub_WoSonly_0524 %>% filter(NOAA_affiliated == 0)

missing_affil <- anti_join(NOAA_omicsPub_WoSonly_0524, NOAA_affiliated, by = "Authors") %>% anti_join(., NOAA_funded, by = "Authors")

ggplot(NOAA_omicsPub_WoSonly_0524 %>% filter(`Publication Year` > 2019), aes(`Publication Year`, fill = as.factor(NOAA_affiliated))) + 
  geom_bar(position = "stack") + scale_fill_manual(labels = c("NOAA funded", "NOAA affiliated"), values = c("gray", "black")) +
  theme_bw() + theme(legend.title = element_blank()) + 
  ggtitle("NOAA 'Omics publications by year")

ggsave("NOAApubs_byaffilationvsfunding_052924.png", dpi = 600)

# word cloud on article titles or keywords?
...
# Create a single string with all keywords using KEYWORDS PLUS
all_keywords_PLUS <- paste(NOAA_omicsPub_WoSonly_0524$`Keywords Plus`, collapse = ";")
...
# Set up the plotting device with a larger size
# Here, we set the size to 12x8 inches, but you can adjust this as needed
png(filename = "wordcloud_052924_keywordsplus.png", width = 12, height = 8, units = "in", res = 300)
...
# Create a single string with all keywords using AUTHOR KEYWORDS
all_keywords <- paste(NOAA_omicsPub_WoSonly_0524$`Author Keywords`, collapse = ";")

# Split the keywords into individual words
keywords_split <- unlist(strsplit(all_keywords, ";"))

# Remove any leading/trailing whitespace
keywords_split <- trimws(keywords_split)

# Create a frequency table of the keywords
keyword_freq <- table(keywords_split)

# Convert the frequency table to a data frame
keyword_freq_df <- as.data.frame(keyword_freq)
keyword_freq_df$keywords_split <- as.character(keyword_freq_df$keywords_split)
keyword_freq_df <- keyword_freq_df %>% filter(keywords_split != "NA")

# Here, we set the size to 12x8 inches, but you can adjust this as needed
png(filename = "wordcloud_052924_AUTHORkeywords.png", width = 12, height = 8, units = "in", res = 300)

# Adjust the margins to be larger
par(mar = c(5, 4, 4, 2) + 0.1)

# Generate the word cloud
wordcloud(words = keyword_freq_df$keywords_split, 
          freq = keyword_freq_df$Freq, 
          min.freq = 4, 
          scale = c(3, 0.5), 
          colors = brewer.pal(8, "Dark2"))
